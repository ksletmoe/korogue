...
class LayeredTilemapTest : FunSpec({
    test("an empty cell has no top tile") {
        LayeredTilemap<StaticTile>(4, 4).topTileAt(0, 0) shouldBe null
    }
    ...
    test("removing from a layer that does not exist is a no-op") {
        ...
        LayeredTilemap<StaticTile>(4, 4).removeTile(0, 0, 3) // must not throw
    }
    ...
    test("moveTile from a layer that does not exist is a no-op") {
        ...
        LayeredTilemap<StaticTile>(4, 4).moveTile(0, 0, 99, 1, 1, 0) // must not throw
    }

...
/**
 * Recomputes each simulated zone's `lightMap` from the [LightEmitter] entities in it,
 ...
 *
 * Zone-scoped: only [gameWorld]'s simulated zones are recomputed; dormant zones keep their last
 * (frozen) lightMap. The scope is the world's `SimulatedZonePolicy` (ADR-0021), not a per-system
 * knob — a game wanting every zone lit sets that policy to return all of them.
 */
class LightingSystem(
    private val gameWorld: GameWorld,
    private val calculators: Registry<LightValueCalculator>,
    private val ambientLight: LightValue? = null,
) : Staged {
    override val stage: PipelineStage get() = StandardStage.LIGHTING
    ...
    override fun update(
        ...
        val target = gameWorld.zones.filterKeys { it in active }
        ...
        for (entity in world.entitiesWith<LightEmitter, Position, ZoneMember>()) {
            ...
            val calculator = calculators.resolve(emitter.calculatorId)

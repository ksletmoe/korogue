...
private data class QuadrantPoint(val quadrant: Quadrant, val row: Int, val column: Int) {
    fun toGridCoordinates(): Point = quadrant.transform(this)
}
...
// tiles
private class Quadrant(val direction: CardinalDirection, val origin: Point) {
    /**
     ...
     */
    fun transform(quadrantPoint: QuadrantPoint): Point {
        return when (direction) {
            CardinalDirection.NORTH -> Point(origin.x + quadrantPoint.column, origin.y - quadrantPoint.row)
            CardinalDirection.EAST -> Point(origin.x + quadrantPoint.row, origin.y + quadrantPoint.column)
            CardinalDirection.SOUTH -> Point(origin.x + quadrantPoint.column, origin.y + quadrantPoint.row)
            CardinalDirection.WEST -> Point(origin.x - quadrantPoint.row, origin.y + quadrantPoint.column)
        }
    }
}
...
class SymmetricShadowCaster : LineOfSightCalculator {
    ...
    private fun castShadows(
        ...
        val gridRectangle = Rectangle(0, 0, tiles.width, tiles.height)
    ...
    private fun limitVisibilityDistance(origin: Point, maxVisibilityDistance: Int, visibilityGrid: Grid<Boolean>) {
        if (maxVisibilityDistance == Int.MAX_VALUE) {
            return
        }

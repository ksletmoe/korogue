# ADR-0001: Split krogue (engine) from kotile (renderer)
...
krogue began as a roguelike with its tile rendering baked in. Rendering (tiles,
...
- **krogue** — a reusable roguelike game engine built on kotile (world/zone model,
...
could plausibly use it, it belongs in kotile.** During development krogue consumes
kotile via Gradle `includeBuild("../kotile")`.

## Consequences

- Clean separation and independent reuse; roguelike-specific helpers (FOV, lighting
  application) stay in krogue, keeping kotile AWT- and roguelike-free.

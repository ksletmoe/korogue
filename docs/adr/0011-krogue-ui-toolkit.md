# ADR-0011: A krogue-side TUI widget toolkit; kotile stays a renderer
...
The original krogue had a multi-pane layout (topbar + sidebar); it was dropped in the
...
The decision (krogue-ui-layout): **where does this live — kotile or krogue — and how?**
...
Build a small **retained-mode TUI widget toolkit in krogue**, on top of kotile's existing
`AsciiTileWindow`. **kotile gains nothing** — it stays a renderer.

- **Draw-surface seam.** Define a thin krogue interface (a cell sink: `put(x, y, z, glyph, fg, bg)`
...
- kotile's public surface stays frozen for its 1.0 publish; every UI concern is added in krogue
...
- **Multiple `AsciiTileWindow`s sharing a `KotileCanvas`** (`createWithCanvas`, which kotile
  ...
  with krogue-owned rects is simpler and sufficient.
